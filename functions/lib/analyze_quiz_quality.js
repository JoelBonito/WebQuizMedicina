...
exports.analyzeQuizQuality = void 0;
const functions = __importStar(require("firebase-functions/v1"));
const admin = __importStar(require("firebase-admin"));
exports.analyzeQuizQuality = functions
    ...
    try {
        ...
        if (userProfilesSnapshot.empty) {
            throw new functions.https.HttpsError('not-found', `Usuário ${email} não encontrado`);
        }
        ...
        const projectsSnapshot = await db
            ...
            .get();
        const project = projectsSnapshot.docs.find((doc) => doc.data().title.toLowerCase().includes(projectName.toLowerCase()));
        if (!project) {
            const availableProjects = projectsSnapshot.docs.map((doc) => doc.data().title);
            throw new functions.https.HttpsError('not-found', `Projeto "${projectName}" não encontrado. Projetos disponíveis: ${availableProjects.join(', ')}`);
        }
        ...
        if (questionsSnapshot.empty) {
            ...
            allTodayQuestions.docs.forEach((doc) => {
                ...
                sessionMap[sessionId].push(Object.assign(Object.assign({ id: doc.id }, questionData), { created_at: questionData.created_at.toDate() }));
            });
            ...
            if (!closestSession) {
                throw new functions.https.HttpsError('not-found', 'Nenhuma sessão de quiz válida encontrada próxima do horário especificado');
            }
            quizQuestions = sessionMap[closestSession].slice(0, 20);
        }
        else {
            quizQuestions = questionsSnapshot.docs.slice(0, 20).map((doc) => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { created_at: doc.data().created_at.toDate() })));
        }
        ...
        const uncoveredTopics = sourceTopicsArray.filter((sourceTopic) => !questionTopicsArray.some((qt) => qt.toLowerCase().includes(sourceTopic.toLowerCase()) ||
            sourceTopic.toLowerCase().includes(qt.toLowerCase())));
        const coveredTopics = sourceTopicsArray.filter((sourceTopic) => questionTopicsArray.some((qt) => qt.toLowerCase().includes(sourceTopic.toLowerCase()) ||
            sourceTopic.toLowerCase().includes(qt.toLowerCase())));
...
//# sourceMappingURL=analyze_quiz_quality.js.map