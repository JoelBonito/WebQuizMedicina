 ...
 */
export async function generateEmbeddings(chunks: Chunk[], modelName?: string): Promise<ChunkWithEmbedding[]> {
    ...
    for (let i = 0; i < chunks.length; i += EMBEDDING_BATCH_SIZE) {
        ...
        try {
            const embeddings = await Promise.all(
                batch.map(chunk => getEmbedding(chunk.content, modelName))
            );
...
export async function semanticSearchWithTokenLimit(
    ...
    const queryEmbedding = await getEmbedding(query, modelName);
