 ...
 */
function getFocusedSummaryTitle(language: string): string {
...
export const generate_focused_summary = onCall({
  ...
  try {
    ...
    if (!projectDoc.exists || projectDoc.data()?.user_id !== userId) {
      throw new HttpsError("not-found", "Project not found or unauthorized");
    }
    ...
    if (difficulties.length === 0) {
      throw new HttpsError("failed-precondition", "No difficulties found. Study with quiz and flashcards first to identify your weak points.");
    }
    ...
    if (sources.length === 0) {
      throw new HttpsError("failed-precondition", "No sources available. Please upload and process sources first.");
    }
...
Respond ONLY with complete, well-formatted HTML. Do not add explanations outside the HTML.`;
    ...
    try {
      result = await callGeminiWithUsage(
        prompt,
        modelName,
        32768, // Increased for comprehensive summaries
    ...
    throw new HttpsError("internal", error.message || "Failed to generate focused summary");
  }
});
