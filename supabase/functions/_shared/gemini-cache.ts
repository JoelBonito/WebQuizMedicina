 ...
 */
export async function getCacheInfo(cacheName: string): Promise<CacheInfo | null> {
  if (!GEMINI_API_KEY) {
    throw new Error('GEMINI_API_KEY not configured');
  }
 ...
 */
export async function isCacheValid(cacheName: string): Promise<boolean> {
  const cacheInfo = await getCacheInfo(cacheName);
 ...
 */
export async function safeDeleteCache(cacheName: string | null): Promise<void> {
  if (!cacheName) {
    return;
  }
