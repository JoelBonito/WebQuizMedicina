-- Final fix: resolve ambiguous columns AND add BRL support
...
-- RPC Function: Get token usage grouped by user (FIXED - with BRL)
...
DECLARE
  usd_to_brl NUMERIC := 5.50; -- Taxa de câmbio USD -> BRL (ajuste conforme necessário)
BEGIN
...
COMMENT ON FUNCTION public.get_token_usage_by_user IS 'Returns token usage statistics grouped by user with BRL conversion (admin only)';

-- RPC Function: Get token usage grouped by project (FIXED - with BRL)
...
COMMENT ON FUNCTION public.get_token_usage_by_project IS 'Returns token usage statistics grouped by project with BRL conversion (admin only)';

-- RPC Function: Get daily usage (FIXED - with BRL)
...
COMMENT ON FUNCTION public.get_daily_usage IS 'Returns daily token usage statistics with BRL conversion (admin only)';

-- RPC Function: Get summary (FIXED - with BRL)
...
COMMENT ON FUNCTION public.get_token_usage_summary IS 'Returns summary statistics with BRL conversion (admin only)';
