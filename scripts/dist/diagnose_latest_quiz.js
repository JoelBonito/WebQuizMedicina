...
async function diagnoseLatestQuiz() {
    try {
        ...
        // Se tiver session_id é melhor, senão usa janela de tempo
        let quizQuestions = [];
        ...
        if (sessionId) {
            ...
            startTime.setMinutes(targetTime.getMinutes() - 5);
            const endTime = new Date(targetTime);
            endTime.setMinutes(targetTime.getMinutes() + 5);
        ...
        summariesSnapshot.docs.forEach((doc, idx) => {
            ...
            if (matches.length > 0) {
                ...
                if (idx === 0)
                    console.log(`   🔎 Amostra do conteúdo: ${content.substring(0, 200)}...`);
            }
        });
        // B. Mindmaps
        ...
        // C. Sources diretos (caso Flashcards ou geração direta)
        // Se ainda não temos tópicos, tentar inferir das questões
        ...
        // Cruzamento
        if (extractedSourceTopics.length > 0) {
            const covered = extractedSourceTopics.filter(st => qTopicsList.some(qt => qt.toLowerCase().includes(st.toLowerCase()) || st.toLowerCase().includes(qt.toLowerCase())));
        ...
        quizQuestions.forEach((q, i) => {
            ...
            if (typeof q.opcoes === 'string') {
                try {
                    cleanOptions = JSON.parse(q.opcoes);
                }
                catch (e) { }
            }
            if (Array.isArray(cleanOptions)) {
                // Heuristica simples: verificar se a letra ou o texto bate
                const match = cleanOptions.some(opt => opt.startsWith(q.resposta_correta) ||
        ...
        if (issues === 0)
            console.log('✅ Nenhuma falha estrutural grave detectada nas questões.');
    }
    catch (error) {
